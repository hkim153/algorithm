...
public class Main
{
    ...
    public static void main(String[] args)
    {
        ...
        {
            System.out.println(main.solution(FRIENDS[cTest], GIFTS[cTest]));
            cTest++;
        }
    }
    ...
    public int solution(String[] friends, String[] gifts)
    {
        ...
        int[][] giveMap = new int[numOfFriends][numOfFriends];
        int [] priorities = new int[numOfFriends];

        for(String gift : gifts)
        {
            StringTokenizer stringTokenizer = new StringTokenizer(gift);
            String give = stringTokenizer.nextToken();
            String receive = stringTokenizer.nextToken();

            giveMap[index.get(give)][index.get(receive)]++;
            priorities[index.get(give)]++;
            priorities[index.get(receive)]--;
        }
        for(int[] test : giveMap)
        {
        ...
        for(int y = 0; y < numOfFriends; y++)
        {
            for(int x = 0; x < numOfFriends; x++)
            {
                if(y != x)
                {
                    if((giveMap[y][x] > giveMap[x][y]) ||
                       (giveMap[y][x] == giveMap[x][y] && priorities[y] > priorities[x]))
                    {
                        next[y]++;
                    }
                }
