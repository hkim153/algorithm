...
/**
 * <a href="https://algospot.com/judge/problem/read/QUADTREE">링크</a>
 * <p>
 * 문제
...
public class Main
{
    public static void main(String[] args) throws IOException
    {
        ...
        while(testCase-- > 0)
        {
            String compression = br.readLine();

            String upDownCompression = solve(compression);
            System.out.println(upDownCompression);
        }
    }

    private static String solve(String compression)
    {
        ...
        if(compression.charAt(0) == 'x')
        {
            int nextIndex3 = findNext(compression, 1);
            String s3 = compression.substring(1, nextIndex3);
            //System.out.println(s3);
            int nextIndex4 = findNext(compression, nextIndex3);
            String s4 = compression.substring(nextIndex3, nextIndex4);
            //System.out.println(s4);
            int nextIndex1 = findNext(compression, nextIndex4);
            String s1 = compression.substring(nextIndex4, nextIndex1);
            //System.out.println(s1);
            String s2 = compression.substring(nextIndex1);
            //System.out.println(s2);
            return 'x' + solve(s1) + solve(s2) + solve(s3) + solve(s4);
        }

        throw new IllegalArgumentException();
    }

    private static int findNext(String compression, int start)
    {
        int expectedCount = 1;
        int count = 0;
        int off = start;
